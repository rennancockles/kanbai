...
When asked to work on the project, follow this loop:
...
5. Repeat until the sprint is empty or the user stops you.
...
name: kanbai-next
description: >-
  Work the kanbai board: pick the next actionable task, move it to "doing", implement it,
  then move it to "done". Use when the user asks to work through the board, pick up the next
  task, or continue development on this project.
allowed-tools: Bash(kanbai *)
---

# Work the next kanbai task

Follow this loop, using the `kanbai` CLI for all board changes (never edit `.kanbai/`
files directly):

1. Run `kanbai next --json`. If it prints `null`, tell the user the backlog is empty or
...
5. Unless the user asked for a single task, repeat from step 1 for the next card.

Report which card you worked on and its outcome after each iteration.
"""
